...
def update_the_truth(truth, df_to_concat):
    df_to_concat.columns = df_to_concat.columns.str.strip()
    truth.columns = truth.columns.str.strip()
    df_to_concat_int = df_to_concat.rename(columns={'UPC':'wine_upc',
    ...
    new_truth = pd.concat([truth,df_to_concat_final],ignore_index=True)
...
def identify_known_upcs(file):
    ...
    corrected_identified_upcs_unique.to_csv(temp_file_11.name, index=False)

    # Try to match unidentified UPC names with known names
    corrected_unidentified_upcs = identify_name_of_unidentified_upc(input_unidentified_upcs)
    corrected_unidentified_upcs.rename(columns={'wine_upc':'Nielsen Item Names ID',
    ...
    upc_message = ("""# Identified UPCs\n"""
            f""" We have already seen the UPCs of **{num_of_identified_rows} rows**: **{num_of_identified_upcs} UPCs** have been identified.\n"""
            f""" Please review the following lines that have been fully filled using the truth.\n"""
            f""" If you decide to modify some of these lines and submit your own correction, the truth will be modified accordingly."""
            )
    
    identified_names_df = corrected_unidentified_upcs[corrected_unidentified_upcs['Suggested name modification']!='']
    unidentified_names_df = corrected_unidentified_upcs[corrected_unidentified_upcs['Suggested name modification']=='']
    ...
    if identified_names_df.shape[0] > 0:
        ...
        return (
            ...
            gr.update(value = result_description), # result_of_corrected_known_upcs_markdown
            gr.update(value=identified_names_df_styled), # name_modifications_proposition_df
            gr.update(value=temp_file_2.name), # name_modifications_proposition_csv
            corrected_identified_upcs_final, # identified_upcs_df_state
            ...
            gr.update(visible=True), # modiify_the_truth_button
        ...
        result_description = """
        **No unidentified UPC could be renamed automatically.**
        """
        return (
            input_df_beautiful_final, # input_csv_df_state
            gr.update(value=upc_message, visible=True), # upc_message
            gr.update(value=result_description), # result_of_corrected_known_upcs_markdown
            None, # name_modifications_proposition_df
            None, # name_modifications_proposition_csv
            corrected_identified_upcs_final, # identified_upcs_df_state
            gr.update(value=corrected_identified_upcs_unique, visible=True), # identified_upcs_df
            gr.update(visible=True), # proceed_with_this_truth_button
            gr.update(visible=True), # modiify_the_truth_button
...
def proceed_with_identified_upcs_fn():

    return (
        gr.update(visible=True), # result_of_corrected_known_upcs_markdown
        gr.update(visible=True), # name_modifications_proposition_df
        gr.update(visible=True), # name_modifications_proposition_csv
        gr.update(interactive=False), # proceed_with_this_truth_button
        gr.update(interactive=False), # modiify_the_truth_button
        gr.update(visible=True), # result_of_corrected_known_upcs_markdown
        gr.update(visible=True), # name_modifications_proposition_df
        gr.update(visible=True), # name_modifications_proposition_csv
...
def data_prep_for_all_input_dfs(input_df,additional_column:str=None,check=True):
    ...
    if check:
        ...
        if input_df['Nielsen Item Names ID'].dtype!='int64':
    ...
    input_df_final['Colors'] = input_df.apply(change_red_to_white,axis=1)
...
def identify_name_of_unidentified_upc(df):
...
def accept_all_changes_fn(identified_names_df_state,
    ...
    try:
        new_truth_state = pd.concat([add_to_truth_df_state,identified_names_df_final_upcs_to_add],ignore_index=True)
    except TypeError:
        new_truth_state = identified_names_df_final_upcs_to_add
    
    message_accept_all_changes = """### These UPCs will be added to the ground truth"""
    ...
    """
    list_of_identified_names = identified_names_df['UPC'].unique().tolist()
    unidentified_names_df_new_state = unidentified_names_df_state[~unidentified_names_df_state['Nielsen Item Names ID'].isin(list_of_identified_names)]
    ...
    return (
        ...
        gr.update(value=message_accept_all_changes, visible=True), # accept_all_changes_markdown
...
def click_own_name_correction_fn():
    ...
    return gr.update(interactive=False), gr.update(interactive=False), gr.update(value=message,visible=True),gr.update(visible=True),gr.update(visible=True)

def csv_input_cleared_fn():
    return (gr.update(visible = False), # upc_message
            gr.update(visible = False), # result_of_corrected_known_upcs_markdown
            ...
            gr.update(visible = False), # final_result_of_corrected_known_upcs_markdown
...
def no_additional_modification_button_fn(identified_upcs_df_state,
    ...
    number_of_rows_with_identified_upcs = identified_upcs_df_state.shape[0]
    identified_upcs_df_unique = identified_upcs_df_state.drop_duplicates(subset=['UPC'])
    number_of_identified_upcs = identified_upcs_df_unique.shape[0]

    number_of_rows_with_identified_names = identified_names_df_state.shape[0]
    
    identified_names_df_unique = identified_names_df_state.drop_duplicates(subset=['UPC'])
    number_of_upcs_with_identified_names = identified_names_df_unique.shape[0]
    
    unchanged_rows = unidentified_names_df.shape[0]
    identified_upcs_df_state_final = identified_upcs_df_state[~identified_upcs_df_state['Item Names'].str.contains('remove', case=False, na=False)].reset_index(drop=True)
    number_of_rows_removed = identified_upcs_df_state.shape[0] - identified_upcs_df_state_final.shape[0]
    ...
    - We already knew the UPCs of **{number_of_rows_with_identified_upcs} rows** - this represents **{number_of_identified_upcs} UPCs**.\n
    - We managed to identify the name of **{number_of_rows_with_identified_names} rows** for which we did not know the UPC - this represents **{number_of_upcs_with_identified_names} UPCs**. These UPCs will be added to the ground truth at the end of the session. They will be identified in the next data cleansing sessions.\n
    - **{number_of_rows_removed} rows** were removed because they were not Champagne.
    - **{unchanged_rows} rows** were unchanged.
    """
    final_df, temp_file4_name = final_df_computation(identified_upcs_df_state_final,identified_names_df_state,additional_modifications_df_state,unidentified_names_df)

    return (
        unidentified_names_df, # unidentified_names_df_state
        gr.update(value = final_result_message, visible=True), # final_result_of_corrected_known_upcs_markdown
        ...
        gr.update(visible=True, value= add_to_truth_df_state),
        gr.update(visible=True)
        ) 
...
def click_confirm_own_correction_fn(inp_correction,
    ...
    list_of_known_upc = identified_upcs_df_state['UPC'].unique().tolist()
...
def final_df_computation(identified_upcs_df_state,identified_names_df_state,additional_modifications_df_state,unidentified_names_df_state):
    
    dfs_to_concat = []
    
    if identified_upcs_df_state is not None:
        dfs_to_concat.append(identified_upcs_df_state)
...
def confirm_additional_modifications_button_fn(inp_additional_modifications,
                                               unidentified_names_df_state,
                                               identified_upcs_df_state,
                                               identified_names_df_state,
                                               truth,
    ...
    Here is a summary of the modifications made:\n
    - We already knew the UPCs of **{number_of_rows_with_identified_upcs} rows** - this represents **{number_of_identified_upcs} UPCs**.\n
    - We managed to identify the name of **{number_of_rows_with_identified_names} rows** for which we did not know the UPC - this represents **{number_of_upcs_with_identified_names} UPCs**. These UPCs were added to the ground truth and will be identified in the next data cleansing sessions.\n
    - **{number_of_rows_removed} rows** were removed because they were not Champagne.
    - **{unchanged_rows} rows** were unchanged.
    """
    final_df, temp_file4_name = final_df_computation(identified_upcs_df_state_final,identified_names_df_state,additional_modifications_df_final,unidentified_names_df_final)

    return (
        identified_upcs_df_state_final,
        ...
        gr.update(visible=True,value=confirm_additional_modifications_message), # confirm_additional_modifications_markdown
        gr.update(value = final_result_message, visible=True), # final_result_of_corrected_known_upcs_markdown
        gr.update(value=final_df,visible=True), # final_df
        gr.update(value=temp_file4_name,visible=True), # final_csv
        new_truth_state,  # add_to_truth_df_state
        gr.update(visible=True),
        gr.update(visible=True, value=new_truth_state),
        gr.update(visible=True))
...
def click_UPCs_to_add_to_truth_button_fn(truth, add_to_truth_df_state):

    if add_to_truth_df_state.shape[0]>0:
        update_the_truth(truth=truth,df_to_concat=add_to_truth_df_state)